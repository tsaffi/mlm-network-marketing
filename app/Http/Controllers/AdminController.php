...
*/
    ...
    public function activateUser(Request $request)
    {
        ...
        try {
          // Set the parent_id to 0(null) if there is only one user in the system.
          // The first user cannot have a parent.
         if (DB::table('users')->count() <= 1) {
            $parent_id = 0;
            DB::table('users')->update(['role'=>'admin']); // change the role of the first user to admin
         }else {
            ...
            // if the referer (parent) does not exists, register him/her under the first user (admin)
            if ($parent){
              $parent_id = $this->getParentId($parent->username);
            }else{
              $parent = DB::table('users')->orderBy('id')->first();
              $parent_id = $this->getParentId($parent->username);
              // update the referer of this user and set it to the first user (admin)
              DB::table('users')->where('id', $node_id)->update(['referrer'=>$parent->username]);
            }
         }
    ...
    public function onlinePaymentActivate($paid_amount)
    {
        ...
        try {
         ...
         if (DB::table('users')->count() <= 1) {
            ...
            // $this->pay($parent, $this->referralBonus, $type);
            // get the id of the referer if it exists else set the admin as the referer
            if ($parent){
              $parent_id = $this->getParentId($parent->username);
            } else {
              $parent = DB::table('users')->where('role', 'admin')->first();
              $parent_id = $this->getParentId($parent->username);
              // update the referer of this user and set it to the first user (admin)
              DB::table('users')->where('id', $node_id)->update(['referrer' => $parent->username]);
            }
